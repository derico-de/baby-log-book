...
- **The config-flow failure taxonomy** — a spent Claim Link, a Lapsed Household at claim time, a URL that is not this app, plain HTTP. The conventions research supplies the vocabulary (form errors vs `ConfigEntryError`, reconfigure vs re-auth); sharpens once [Re-binding a Hub that lost its session](issues/04-re-binding-a-hub.md) lands.
...
- **The README a stranger follows** — install, claim, revoke; spec-assembly territory.
