...
- **A Target coming round is the only thing that reaches for a Member's attention** ([ADR-0029](../../docs/adr/0029-the-bottle-chime-is-a-device-setting.md), [ADR-0031](../../docs/adr/0031-a-notice-is-a-target-reached-and-the-household-states-the-offset.md)). Three Notices and no fourth: a bottle nearly out, a Feed due, a Wake Window up — each a number a Member typed, reached. The Feed Interval and the Wake Window are said at an offset the Household states (or never); the bottle's is fixed. Neither of the two interrupts what it is about: no Feed Notice while she is feeding, no Sleep Notice while she is asleep. §9.6 has the delivery.
- **The Bottle Chime is what that sounds like on a phone in the room** ([ADR-0029](../../docs/adr/0029-the-bottle-chime-is-a-device-setting.md)). Ten minutes before a started bottle's Life runs out, a Device *that has been switched on for it* plays two soft notes — so the rest of the bottle can be offered while there is still time. A **Device Setting**, off by default (§9.4): the same bottle is open on every Device, and the phone in the room with the baby is not the one on a sleeping partner's bedside table. The lead is fixed at ten minutes, clamped to half the Life, and is not a fifth setting. Nothing else changes: no badge, no banner, no second colour, nothing written and nothing synced, and no claim about the milk. When the app is not open, the same reminder arrives as a **push notification** ([ADR-0030](../../docs/adr/0030-a-push-is-the-bottle-chime-reaching-a-sleeping-phone.md), §9.6) — one reminder with two ways of arriving, off on both counts until this phone asks.
...
**One FAB, bottom-right in thumb reach.** Tapping it **expands it in place into a fan of six direct actions that curves up and around the button**: **Pee & poop · Sleep · Feed · Tummy time · Measurement · Milestone**.

**A row is a form, not an entry type** ([ADR-0035](../../docs/adr/0035-a-fan-row-is-a-form-not-an-entry-type.md)). Six rows carry eight entry types — breast, bottle and meal all sit behind the one *Feed* row — and a new entry type must justify a new **form** before it may claim a row. The fan is the app's only create-from-nothing surface, so there is no admission test and never was; the obligation is on the form. The corollary: a row leaves the fan only by being given another create surface, which is why Measurement and Milestone stay.

**The ceiling is eight rows**, measured at 360×780 (Galaxy S22, installed) and asserted in `render.test.ts` as arithmetic over the fan's constants, so a seventh and eighth row are free and a ninth trips CI. It was six until the fan stopped stacking straight above the FAB — the row at the thumb now sits *beside* the button and the rows above swing back to the right edge along an arc, reclaiming 74px — and until Pico's inherited 16px button margin, 80px of dead space across six rows, was deleted. A two-column grid of tiles was recommended by [ticket 27](issues/27-the-fan-is-out-of-room.md) and proved unnecessary: the pill, its label, its sub-line and its type colour all survive.
...
~~The screen answers them in this order down one scroll.~~ ~~**Three tabs: Day, Week, Trends** — the grid's two windows, then the cards.~~ **Three tabs: Trends, Week, Month — and Trends is the first and the default.** The one scroll was ticket 28's shape and it hid the cards a screen and a half under a 24-hour axis; the switcher is its 2026-08-22 amendment. Amended again **2026-09-08**: *is this getting better* is the question somebody arrives with, so the cards go first; the Day view is dropped, because what one day held is the timeline's question and the grid's own job starts at seven columns; and a Month — four whole weeks, so stepping keeps every weekday in the same column — is where a drift too slow to see in a week shows up.

**Measurements are not on the grid** (2026-09-08). A weight is a fact about a Baby and not about a time of day, so a disc at 14:20 on a Tuesday said nothing the grid exists to say. Weight and height are cards in Trends instead — a line over her whole life rather than seven bars, because a Baby is weighed at a check-up and a rolling week of measurements is one column and six gaps.

**The curve starts at birth, and birth weight is a Measurement** (2026-09-08). Her weight and length on the day she was born are offered on the Baby in settings, beside the birth date — and what that writes is an ordinary Measurement Entry dated that day, not a field on the Baby. It is the same fact the four-month check-up records; a column of its own would have put the first point of the growth curve somewhere the growth curve does not read from. Which Entry it is falls out of the birth date, derived like first exposure, so a birth date somebody corrects takes the measurement with it. Both fields are optional: a Baby who joins the log at four months has a birth weight nobody may have to hand, and a curve that begins at the first check-up is a true curve, only a shorter one.
...
2. **Sleep** — total per day, split Night Sleep vs Naps (computable only because §7.2 settled which is which). Longest stretch as the secondary number. **Amended 2026-09-08**: the split is *drawn*, not only stated — the column is Night at its base and Nap above it, in two shades of the Sleep hue, on the grid as well as the card, with both figures for today beside the average. The same eleven hours as one night and as six naps are not the same day, and the total hid which half moved. The two shades are named in words beside them and in every column's hidden list, so nothing is knowable from the colour alone.
...
- **Instants are a tick, not a block.** Pee & poop, Meals and Milestones have one time and no duration, so a minimum-height block would be the grid lying about a length they do not have. They are full width like everything else, and never nudged off the time they happened at — two on the same minute are drawn on the same minute, which the column's own list then reads out in order. ~~Two too close to share the trailing edge pack against it rather than spreading across the column.~~ **Dropped 2026-09-08 with the Day view**, which was the only column wide enough to spread them into.
...
The Bottle Chime's push subscription is the same rule taken to its end: there is no notification preference at all, because the subscription *is* the setting (§9.6). The test is whether the setting answers a question about *this phone* rather than about the Household — and it is the test that puts the two **Notice Offsets** on the *other* side of the line, on Household, since the server holds the timer and how much warning is useful is a fact about the routine (ADR-0031).
...
### 9.6 Push — the three Notices this app sends

[ADR-0030](../../docs/adr/0030-a-push-is-the-bottle-chime-reaching-a-sleeping-phone.md), [ADR-0031](../../docs/adr/0031-a-notice-is-a-target-reached-and-the-household-states-the-offset.md). Three **Notices**, and each is one of the Household's own Targets coming round: a **bottle nearly out** (ten minutes, fixed), a **Feed due**, and a **Wake Window up**. Nothing else notifies — no Stale Session, no Measurement, nothing the app decided by itself. Every Notice states a number a Member typed.

- **The subscription is the setting.** One row per Device, alive exactly while that phone's chime is on and its browser has granted permission. Switching the chime off deletes it, signing out deletes it, removal deletes it (a notification is access), and Erasure takes it with the Household. There is no `notify` flag anywhere, which is §2's constraint kept by having nothing to keep in step. It is the switch for all three: whether a phone is woken **at all** is the Device's question.
- **When it is said is the Household's question, and it is the one setting here.** The Feed Notice comes a stated number of minutes *before* the Feed Interval is up; the Sleep Notice a stated number *after* the Wake Window is. Either can be *never*. They live on Household and travel as Revisions like the Day Start, because the server holds the timer and cannot read a Device Setting — and *how much warning is useful* is a fact about the routine, the same answer on Oma's phone as on mum's. Offered as a short list, never a free number. The bottle's ten minutes stay fixed (ADR-0029).
- **The due instants are computed once, in the domain.** `liveNotices` composes `dueInstant`, `anchorEntry` and `bottlesNearingEnd` — the same folds the sticky header draws from — so the server and every Device agree by construction. §6.5's promise, paid twice over.
- **A Notice never interrupts what it is about.** No Feed Notice while a Feed is running; no Sleep Notice while she is asleep, because a Wake Window is time not covered by a Sleep (§8.5).
- **Anchored to an Entry, and said once per Device per kind.** The bottle, the previous Feed, the last Sleep. Logging the next Feed re-anchors the Notice, which is how it comes round again; the kind is part of the key because a bottle nearly out is also the Feed the next interval measures from.
- **A late Notice is never shown.** Each carries the instant it stops being true: the bottle's own due instant, and a fifteen-minute window for the other two. That instant is the push's TTL — never a flat quarter of an hour — so a push service holding a message past it drops it, and the worker re-checks before showing anything. A stopped Feed is silent at every layer: the plan is re-read from the log on every tick, the TTL outlives nothing, and the worker has the last word.
- **Payloads are encrypted to the Device** (RFC 8291, aes128gcm), written against the RFCs with `node:crypto` rather than a dependency. The push service forwards ciphertext, which is what makes it safe for the notification to name the Baby. The text is composed on the server in the Member's own language and says the stated offset out loud, because *due in ten minutes* and *due now* are different facts.
...
- **Only a delivered notice is recorded**, so a push service having a bad minute is retried while the Notice is still true; a 404 or 410 drops the subscription and nothing else does.
...
**Push notifications.** Built, and now for three things — a bottle nearly out, a Feed due, a Wake Window up (§9.6, [ADR-0030](../../docs/adr/0030-a-push-is-the-bottle-chime-reaching-a-sleeping-phone.md), [ADR-0031](../../docs/adr/0031-a-notice-is-a-target-reached-and-the-household-states-the-offset.md)). iOS delivers web push **only to installed PWAs**, so the install nudge is the precondition on an iPhone and Settings says so where a Member can act on it. What stays open is the **Stale Session**, which is the one reminder here that is not a Target coming round: the app does not know a Sleep was forgotten, it only suspects, and the passive banner is still the honest shape for a suspicion.
