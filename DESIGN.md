...
- Entry-type tokens carry four roles per type and appearance — `--t-<type>` (solid fill), `--t-<type>-ink` (on-fill ink: white in day/night, capped at L 0.78 in deep), `--t-<type>-soft` (tint) and `--t-<type>-soft-ink` (ink on the tint). Seven hues, evenly spread, all skipping the accent band: feed rose 15°, tummy time moss 100°, meal green 150°, nappy teal 215°, measurement blue 250°, sleep indigo 285°, milestone plum 330°. Moss was the last gap left between the accent's upper edge and meal green, so an eighth type reopens the palette rather than picking a leftover (ADR-0027). Deep night dims fills to L≈0.35 with chroma halved. Every pair is contrast-verified ≥4.5:1. Glyph and label stay on every surface, so identification never *depends* on the hue (ADR-0026); overdue never adopts a category hue.
- Components reach the type colour through one attribute: `data-t="feed|sleep|nappy|meal|tummy|measure|milestone"` resolves the generic `--t` / `--t-ink` / `--t-soft` / `--t-soft-ink` roles (plumbing at the top of `components.css`). Colour lands on the timeline glyph discs, fan pills, filter type-chips (soft tint idle → solid fill active), the live grid's running bar and live-pills, stat-card bars and marks, the day grid's blocks and marks, and typed sheet headers; forms stay Pico-neutral.
- The **day grid** is the densest use of the seven hues in the app, and the guards are what make that safe: the legend names every hue with its glyph and its word, every week column carries a visually-hidden ordered list of its Entries, the day view labels its blocks, and the accent appears exactly once on the screen — the now line. Its legend chips are deliberately quieter than the filter rail's (hue in the glyph disc, tint on the chip) because all seven start pressed at once, which is the one arrangement solid fills turn into confetti.
...
- Tap floor `--tap: 48px`; the FAB is 62px, fan items 54px on a 62px pitch. The fan **curves around the FAB** rather than stacking above it — the row at the thumb sits beside the button, the rows above swing back to the right edge on a `cos(t·π/2)` arc carried by a per-row `--dx` — and all rows share one width, or both edges go ragged and the pills read as a scatter. **Ceiling: eight rows**, measured at 360×780 and asserted in `render.test.ts` ([ADR-0035](docs/adr/0035-a-fan-row-is-a-form-not-an-entry-type.md)).
...
Pico v2 is the framework, imported into `@layer framework` so app styles never out-specify it; 63 of its 149 tokens are mapped in `tokens.css`. **Two Pico defaults bite anything new** and are worth knowing before debugging them again: it ships a `.grid` utility that turns an element into an auto-fit column grid above 768px (which is why the day grid's classes are `daygrid-*`), and it gives every `button` a 16px bottom margin, which silently adds a row of dead space to any custom control built out of buttons — the fan paid this six times over, 80px, until [ADR-0035](docs/adr/0035-a-fan-row-is-a-form-not-an-entry-type.md), so `margin: 0` is not optional decoration on a stack of buttons. Custom components live in `src/styles/components.css`: app shell + header, the live grid (sleep | feed) with its quiet nappy line; each column closes with a 5px bar above the nappy line — hairline grey while idle, the live colour while that column's session runs — which also serves as the grid's separator, sync status line, timeline rows, stale-Sleep banner, FAB + fan (FAB at `bottom: 76px` clearing the tab bar), bottom sheet, segmented control (`[role=tab]`), toast, bottom tab bar (3 destinations), chips + filter rail, stat cards (hand-drawn bars, numbers stated as text), settings, the day grid (hour gutter, sticky weekday strip, full-width Entries in three layers — sleep ground, sessions, instants — and the now line) with its legend, and the chrome-less claim page.
