...
**Feed Interval**:
The Target for feeding, measured from the previous Feed's start. The one Target a Night Period moves.
...
**Bottle Chime**:
The reminder a Device gives ten minutes before a started bottle's Life runs out, so the rest of it can be offered while there is still time. Two soft notes when the app is open, and a notification when it is not (ADR-0030) — one thing with two ways of arriving, because a phone in a pocket at 3am is the case it exists for. A Device Setting and off until someone switches it on: the same bottle is open on every Device, and whether a sound is wanted is a question about the phone in the room rather than about the Household. It sounds once per bottle, and says nothing the row was not already saying — the countdown addressed to someone who is not looking at it. Switching it on is also what subscribes this phone to the Household's other Notices; the sound itself is still the bottle's alone.
_Avoid_: Alarm, alert, timer going off

**Notice**:
One thing the deployment wakes a Device to say, and never a number the app decided: a Target the Household stated has come round. Three of them — a bottle nearly out ([ADR-0029](docs/adr/0029-the-bottle-chime-is-a-device-setting.md)), a Feed due, a Wake Window up. Each is anchored to the Entry it was measured from, said once per Device, and carries the instant it stops being true — a Notice that arrives late is not a reminder, it is a wrong statement about the Baby, so a push service holding one past that instant drops it. The two that a Household can time state their **Notice Offset**; the bottle's ten minutes cannot be timed and are not a setting.
_Avoid_: Alert, alarm, reminder setting, notification preference

**Notice Offset**:
How far from the due instant a Feed Notice or a Sleep Notice is said — the Feed one *before* the interval is up, the Sleep one *after* the Wake Window is. A Household setting, stated once for everyone: the server holds the timer and cannot read a Device Setting, and how much warning is useful is a fact about the routine rather than about the phone. Either can be set to never. Whether a phone is woken at all is a different question, answered by the Bottle Chime switch on that phone.
_Avoid_: Lead time, snooze, grace period, threshold
